...
#[cfg(unix)]
fn fake_python_fastdb_typescript_generator(tempdir: &tempfile::TempDir) -> PathBuf {
    let path = tempdir.path().join("python-fake-fastdb-typescript");
    let script = r#"#!/bin/sh
if [ "$1" != "-m" ] || [ "$2" != "c_two.fastdb.typescript" ] || [ "$5" != "--schema" ]; then
  echo "unexpected fastdb typescript generator args: $*" >&2
  exit 1
fi
cat > "$4" <<'C_TWO_FASTDB_TYPESCRIPT'
// fake C-Two FastDB helper generated by c_two.fastdb.typescript
export const GENERATED_C_TWO_FASTDB_HELPER = true;
C_TWO_FASTDB_TYPESCRIPT
"#;
    std::fs::write(&path, script).unwrap();
    let mut permissions = std::fs::metadata(&path).unwrap().permissions();
    permissions.set_mode(0o755);
...
fn contract_codegen_typescript_fastdb_schema_runs_c_two_helper_generator() {
    ...
    let python = fake_python_fastdb_typescript_generator(&tempdir);
