...
| ipc-v2 | 64B | 100 | 0.183 | 0.373 | 0.775 | 0.841 | 0.6 MB/s |
| ipc-v2 | 1KB | 100 | 0.213 | 0.369 | 0.472 | 0.517 | 8.4 MB/s |
| ipc-v2 | 64KB | 100 | 0.241 | 0.417 | 0.551 | 0.551 | 477 MB/s |
| ipc-v2 | 1MB | 100 | 0.419 | 0.616 | 0.842 | 0.898 | **4.3 GB/s** |
| ipc-v2 | 10MB | 20 | 2.221 | 2.817 | 2.902 | 2.923 | **8.4 GB/s** |
| ipc-v2 | 100MB | 5 | 16.473 | 26.318 | 27.598 | 27.918 | **10.2 GB/s** |
| ipc-v2 | 512MB | 3 | 287.349 | 333.658 | 337.775 | 338.804 | **3.4 GB/s** |
| ipc-v2 | 1GB | 3 | 724.404 | 835.181 | 845.027 | 847.489 | **2.6 GB/s** |
...
> IPC v2: UDS 控制面 + SharedMemory 数据面 + Pool SHM + 4KB 内联阈值

| 负载 | memory P50 | ipc-v2 P50 | 延迟提升 | memory 带宽 | ipc-v2 带宽 | 带宽提升 |
|------|-----------|-----------|---------|-----------|-----------|---------|
| 64B | 30.97ms | 0.18ms | **169x** | ~0 | 0.6 MB/s | — |
| 1KB | 28.03ms | 0.21ms | **132x** | 0.1 MB/s | 8.4 MB/s | **84x** |
| 64KB | 27.99ms | 0.24ms | **116x** | 4.2 MB/s | 477 MB/s | **113x** |
| 1MB | 34.55ms | 0.42ms | **82x** | 58.6 MB/s | 4.3 GB/s | **76x** |
| 10MB | 60.81ms | 2.22ms | **27x** | 366 MB/s | 8.4 GB/s | **24x** |
| 100MB | 206.69ms | 16.47ms | **13x** | 948 MB/s | 10.2 GB/s | **11x** |
| 512MB | 1,019.64ms | 287.35ms | **3.5x** | 1.0 GB/s | 3.4 GB/s | **3.4x** |
| 1GB | 2,641.72ms | 724.40ms | **3.6x** | 774 MB/s | 2.6 GB/s | **3.5x** |

### IPC v2 vs Thread 对比

| 负载 | thread P50 | ipc-v2 P50 | ipc-v2/thread | thread 带宽 | ipc-v2 带宽 |
|------|-----------|-----------|--------------|-----------|-----------|
| 64B | 0.025ms | 0.183ms | 7.3x slower | 4.7 MB/s | 0.6 MB/s |
| 1MB | 0.214ms | 0.419ms | 2.0x slower | 9.0 GB/s | 4.3 GB/s |
| 10MB | 1.944ms | 2.221ms | 1.1x slower | 9.9 GB/s | 8.4 GB/s |
| 100MB | 13.72ms | 16.47ms | 1.2x slower | 10.5 GB/s | 10.2 GB/s |
| 1GB | 577.82ms | 724.40ms | 1.3x slower | 3.6 GB/s | 2.6 GB/s |

### IPC v2 关键发现

1. **小消息延迟：169x 提升** — memory 的 ~31ms 固定开销（watchdog + 文件轮询）被 IPC v2 的 UDS 阻塞 recv 消除，降至 0.18ms
2. **中等负载带宽峰值 10.2 GB/s @ 100MB** — Pool SHM 零 syscall + memcpy 直写，逼近 thread:// 的 10.5 GB/s（97%）
3. **大负载（≥512MB）受内存压力影响** — RSS 达 8.4GB，3.14t GC 和内存压力导致 512MB/1GB 降至 3.4/2.6 GB/s
4. **瓶颈交叉点从 ~10MB 降至 ~64KB** — IPC v2 固定开销仅 0.18ms，64KB 时协议开销已降至 76%
5. **≥10MB 时仅比 thread:// 慢 1.1-1.3x** — 验证了 UDS + SHM 双平面设计的有效性
