...
impl ShmRegion {
    ...
    pub fn create(name: &str, size: usize) -> Result<Self, String> {
        ...
        unsafe {
            // An existing name belongs to another backing incarnation. Never
            // unlink or resize it while a peer may still hold that mapping.
    ...
    pub fn open(name: &str, expected_size: usize) -> Result<Self, String> {
        if expected_size == 0 || expected_size > isize::MAX as usize {
            return Err("shared memory size must be between 1 and isize::MAX".to_string());
        }
        let c_name = CString::new(name).map_err(|e| e.to_string())?;
        ...
        unsafe {
            ...
            let actual_size = match usize::try_from(stat.st_size) {
                Ok(size) if size > 0 && size <= isize::MAX as usize => size,
                _ => {
                    libc::close(fd);
                    return Err("shared memory backing has an invalid size".to_string());
                }
            };
...
mod tests {
    ...
    fn duplicate_create_preserves_existing_mapping() {
        let name = format!("/c2dup_{}", std::process::id());
    ...
    #[test]
    fn open_validates_full_backing_size() {
        let name = format!("/c2full_{}", std::process::id());
        ...
        assert!(ShmRegion::open(&name, reader.size() + 1).is_err());
    }

    #[test]
    fn invalid_size_fails_before_creating_backing() {
        let name = format!("/c2zero_{}", std::process::id());
        assert!(ShmRegion::create(&name, 0).is_err());
        assert!(ShmRegion::open(&name, 1).is_err());
    }
}
