...
Use `createBundledC2MemFfiNodeRuntime()` for the normal generated-client composition. It returns the real Node Unix-socket `connect` adapter and a bundled response-pool factory accepted by generated `createC2MemFfiNativeBuddyResponseShmReader(...)`; callers do not need to load native symbols or assemble that response-pool adapter themselves.
...
This facade owns only C-Two IPC memory-pool access and byte-visible request/response backing. It does not interpret any payload format or grant payload semantics to an allocator object. Portable FastDB adapters are composed separately from Core-generated artifacts and currently open owned payload copies; a future direct-backing path must use the public FastDB backing contract and prove resource-time construction before making a stronger claim.
