...
> **Vision context**：本文是 C-Two RPC 的 SOTA 设计模式笔记，重点在"隐式服务 /
> 注册-获取 / 资源托管"这一核心范式。该范式构成了 [`endgame-architecture.md`](./endgame-architecture.md)
> 中 L1（c-two 协议层）的设计依据，特别是 §7 "建模扩展 = 组合 CRM 的客户端" 中关于
> **CRM 既参与计算又对外服务** 的论述直接源自本文。
...
- C-Two 进程级注册器
```python
cc.register(namespace_as_address, crm, crm_data, shutdown_callback)    # 可能还有其他参数
```
...
```python
# 获取资源对象的引用
crm_ref: CRM_CONTRACT = cc.connect(namespace_as_address)    # 可能还有其他参数
# 关闭资源对象的引用
...
- C-Two 线程优惠
```python
crm_instance: CRM_CLASS = cc.connect(crm, namespace_as_address)    # 可能还有其他参数
```
...
`cc.register(namespace, crm, crm_data)` + `cc.connect(namespace)`本质上是一个**进程内/进程间的服务注册表（Service Registry）**。
...
- **版本兼容性**：当前 CRM 契约已有`version`字段（语义版本），但SOTA设计中`cc.connect()`没有显式传入版本要求。在耦合场景中可能导致版本不匹配的隐蔽bug。
...
| 维度 | gRPC | SOTA C-Two |
|------|------|-----------|
| 接口定义 | Protobuf IDL (.proto文件) | Python装饰器 (@crm) |
...
```python
cc.register('ns1', Grid, grid)      # 注册到同一个Server
...
建议`cc.connect()`始终返回代理对象（Proxy），而非在线程内返回原始实例、跨进程返回 CRM stub：
...
`cc.register(namespace, crm, crm_data, shutdown_callback)`中的`shutdown_callback`需要明确：在`unregister()`时调用？进程退出时调用？远程`shutdown`信号到达时调用？这些场景的语义应予以区分。
...
2. **前端可视化直访大payload**：C-Two的目标之一是资源一致性访问。当接入fastdb并支持TypeScript 的 CRM codegen + HTTP client后，前端（如WebGL/WebGPU可视化引擎）将直接通过HTTP访问CRM资源。GIS可视化中，一次tile请求返回的mesh/raster数据可能是几十MB甚至更大。链路为：
