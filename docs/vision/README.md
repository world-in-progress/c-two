...
- **C-Two 贡献者**：把 `vision/` 当作协议边界的约束来源。任何新特性在进入 `plans/` 之前，都应该能在 vision 里找到对应的动机。
- **Toodle / Gridmen / 下游框架开发者**：`endgame-architecture.md` 刻画了你们赖以构建的运行时画像。如果你发现某个边界划错了，issue 提上来，我们一起修正 vision。
- **Agent / Extension 作者**：`endgame-architecture.md` 的第 7 节专门讨论 CRM 契约作为 Agent Tool Schema 的用法，以及人在回路的约束；第 3 节的"计算模型"解释了 Client / CRM / 复合进程三种角色的边界。
