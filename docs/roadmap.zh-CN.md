...
| 能力 | 尚未完成 | 实现方向 |
| --- | --- | --- |
| FastDB-first CRM ABI、bridge 与 codegen foundation | C-Two 现在拥有 `c_two.fastdb`、FastDB call-db planning、bridge derivation、通过 `c3 contract codegen typescript --fastdb-schema` 生成 FastDB-backed TypeScript helper，以及跨 transport 的 FastDB ABI 冒烟验证。FastDB 仓库保留 generic schema/storage/serialization/view/runtime substrate。 | Portable CRM payload 只走 FastDB call-db 或 no-payload；pickle 只保留为 Python-only fallback；C-Two runtime 的 route/relay/IPC/scheduler/lease 层仍不解析 FastDB 存储内部结构。 |
