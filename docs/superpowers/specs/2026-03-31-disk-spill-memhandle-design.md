...
```
                  ...
                  └────────┬────────────┘
                           │
                  ┌────────▼────────────┐
                  │ should_spill(size)?  │  ← 查询可用物理内存
                  └────┬───────────┬────┘
                   yes │           │ no
              ┌────────▼──┐  ┌────▼───────────┐
              │ FileSpill │  │ size ≤ buddy    │
              │ (磁盘兜底) │  │ max_block?     │
              └───────────┘  └──┬──────────┬──┘
                            yes │          │ no
                       ┌────────▼──┐  ┌───▼──────────┐
                       ...
                       └─────┬─────┘  └──────────────┘
                             │ fail?
                        ┌────▼──────────┐
                        │ FileSpill     │  ← buddy 耗尽时也走文件兜底
                        │ (回退)        │
                        └───────────────┘
```

1. **首先检查内存压力**: `should_spill(size, threshold)` → 若 `size > available_ram * threshold`，直接走 FileSpill（不浪费 SHM 资源）
2. **RAM 充足时按大小分派**: 小块 → buddy, 大块 → dedicated
3. **buddy 耗尽兜底**: buddy alloc 失败 → 回退到 FileSpill
