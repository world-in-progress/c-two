...
class TestPoolAcquireRelease:
    ...
    def test_acquire_creates_client(self, ipc_v3_addr, pool):
        client = pool.acquire(ipc_v3_addr)
        assert isinstance(client, SharedClient)
        assert pool.active_count() == 1
        assert pool.refcount(ipc_v3_addr) == 1
        pool.release(ipc_v3_addr)

    def test_acquire_reuses_client(self, ipc_v3_addr, pool):
        c1 = pool.acquire(ipc_v3_addr)
        c2 = pool.acquire(ipc_v3_addr)
        assert c1 is c2
        assert pool.refcount(ipc_v3_addr) == 2
        pool.release(ipc_v3_addr)
        pool.release(ipc_v3_addr)

    def test_release_grace_period(self, ipc_v3_addr, pool):
        """After release, client stays alive during grace period."""
        pool.acquire(ipc_v3_addr)
        pool.release(ipc_v3_addr)
        # Still alive immediately after release.
        assert pool.has_client(ipc_v3_addr)

    def test_release_destroys_after_grace(self, ipc_v3_addr, pool):
        """After grace period, client is destroyed."""
        pool.acquire(ipc_v3_addr)
        pool.release(ipc_v3_addr)
        # Wait for grace period (0.5s) plus margin.
        time.sleep(1.0)
        assert not pool.has_client(ipc_v3_addr)

    def test_reacquire_cancels_destroy(self, ipc_v3_addr, pool):
...
class TestPoolConcurrency:
    ...
    def test_concurrent_acquire_release(self, ipc_v3_addr, pool):
...
class TestPoolFunctionality:
    ...
    def test_call_through_pool(self, ipc_v3_addr, pool):
    ...
    def test_shutdown_all(self, ipc_v3_addr, pool):
